...
if __name__ == '__main__':
    ...
    if sys.argv[1:] and sys.argv[1] not in ('-h', '--help'):
        ...
        instance = Instance("--demux ffmpeg --codec cedar --vout cedarfb --no-osd")
        try:
...
# PHW         player.video_set_marquee_int(VideoMarqueeOption.Enable, 1)
# PHW         player.video_set_marquee_int(VideoMarqueeOption.Size, 24)  # pixels
# PHW         player.video_set_marquee_int(VideoMarqueeOption.Position, Position.Bottom)
# PHW         if False:  # only one marquee can be specified
# PHW             player.video_set_marquee_int(VideoMarqueeOption.Timeout, 5000)  # millisec, 0==forever
# PHW             t = media.get_mrl()  # movie
# PHW         else:  # update marquee text periodically
# PHW             player.video_set_marquee_int(VideoMarqueeOption.Timeout, 0)  # millisec, 0==forever
# PHW             player.video_set_marquee_int(VideoMarqueeOption.Refresh, 1000)  # millisec (or sec?)
# PHW             ##t = '$L / $D or $P at $T'
# PHW             t = '%Y-%m-%d  %H:%M:%S'
# PHW         player.video_set_marquee_string(VideoMarqueeOption.Text, str_to_bytes(t))
